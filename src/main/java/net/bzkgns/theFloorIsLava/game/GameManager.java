...
import net.bzkgns.theFloorIsLava.game.currency.MoneyManager;
...
import net.bzkgns.theFloorIsLava.utils.SpreadEntityManager;
...
public class GameManager {
    ...
    private final RisingManager risingManager;
    ...
    public GameManager() {
        ...
        ConfigManager<RisingConfig> dangerConfigManager = ConfigLoader.load(
                new RisingConfig()
        );
        ...
        this.risingManager = new RisingManager(dangerConfigManager);
    ...
    private void startRunningPhase(TheFloorIsLava plugin, Map<UUID, Location> positions) {
        ...
        if (!risingManager.startPreparation()){
    ...
    public boolean earlyStartRisingPhase() {
        ...
        if (risingManager.getState() != RisingManager.DangerState.PREPARATION) return false;
        cancelBossBarTask();
        startRisingPhase();
    ...
    public boolean isGameWinning() {
        ...
        if (risingManager.getState() == RisingManager.DangerState.PREPARATION) {
            return false;
        }
    ...
    public RisingManager getDangerManager() {
        return risingManager;
    }
