...
import net.bzkgns.theFloorIsLava.config.rising.RisingConfig;
import net.bzkgns.theFloorIsLava.config.rising.RisingConfigKeys;
...
public class RisingManager {
    ...
    private final ConfigManager<RisingConfig> dangerConfigManager;
    ...
    public RisingManager(ConfigManager<RisingConfig> dangerConfigManager) {
    ...
    private void resetRuntimeState() {
        dangerLevel = dangerConfigManager.getInt(RisingConfigKeys.START_LEVEL);
    ...
    public boolean resume() {
        ...
        damageTask = Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, this::tickDamage, 0, dangerConfigManager.getInt(RisingConfigKeys.DAMAGE_EVERY));
        if (dangerConfigManager.getBoolean(RisingConfigKeys.SHOW_ALERT) ) {
            particleTask = Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, this::tickParticles, 0, DISPLAY_PERIOD);
        }
        if (dangerConfigManager.getBoolean(RisingConfigKeys.PLACE_LAVA)) {
    ...
    private void tickDamage() {
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.getLocation().getY() < dangerLevel) {
                p.damage(dangerConfigManager.getDouble(RisingConfigKeys.DAMAGE));
            }
        }
    }
    ...
    private void tickPlaceLava() {
        int increaseSize = dangerConfigManager.getInt(RisingConfigKeys.INCREASE_SIZE);
        ...
        int lavaMargin = dangerConfigManager.getInt(RisingConfigKeys.LAVA_MARGIN);
    ...
    public void startRising() {
        ...
        damageTask = Bukkit.getScheduler().scheduleSyncRepeatingTask(plugin, this::tickDamage, 20, dangerConfigManager.getInt(RisingConfigKeys.DAMAGE_EVERY));
