...
public class GamblingInstance {
    ...
    public GamblingInstance(Player player, Price bet) {
        ...
        gamblingTpTaskId = player.getServer().getScheduler().scheduleSyncRepeatingTask(TheFloorIsLava.getInstance(), this::tpElements, 0L, 1L);
        gambleStopTaskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(TheFloorIsLava.getInstance(),this::stopGambling, 1L,1L);
    }
    ...
    public void destroy() {
        ...
        TheFloorIsLava.getInstance().getGameManager().getMoneyManager().addBalance(player.getUniqueId(), gain);
        destroyElements();
    }
